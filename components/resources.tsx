 ...
 * logo wall and the Why tiles. After
 * that each runs at its own rate against the scroll, the third fastest, so
 * the row never sits quite flat.
 */
      ...
      <Container>
        ...
        <Stagger className="mt-[52px] grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-6">
          {resources.items.map((item, index) => (
            <Parallax key={item.title} y={[10, 24, 40][index] ?? 10} className="flex">
            <a
              href={item.href}
              data-stagger
              ...
              className="flex flex-1 flex-col overflow-hidden rounded-[3px] border border-rule text-ink transition-[border-color,transform] duration-[250ms] hover:border-oxblood hover:[transform:translateY(-3px)] active:[transform:none] active:scale-[0.985] active:duration-100 motion-reduce:hover:[transform:none]"
            >
