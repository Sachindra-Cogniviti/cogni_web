 ...
 * diamond used for the location pins in the global presence section.
 */
export function CoupaTraining() {
  return (
    <section
      id="training"
      className={`border-b border-rule bg-paper-alt ${sectionPadding}`}
    >
      <Container className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] items-center gap-[clamp(40px,5vw,80px)]">
        <div data-reveal="0">
          <div className="inline-flex items-center gap-[10px] rounded-[2px] border border-oxblood px-4 py-2 font-mono text-[10.5px] font-medium tracking-[0.18em] text-oxblood uppercase">
            <span className="size-[7px] rotate-45 bg-oxblood" />
            {training.badge}
          </div>

          <h2 className="mt-[26px] max-w-[18ch] text-[clamp(30px,3.4vw,48px)] leading-[1.08] font-semibold tracking-[-0.03em] text-balance">
            {training.heading}
          </h2>
        ...
        <div
          data-reveal="120"
          className="overflow-hidden rounded-[3px] border border-rule-strong bg-paper"
        >
