...
|---|------|---------|----------|
| 1 | URL shape | Nearly every internal link omits the trailing slash → a 308 on every click and every crawl hop | **P0** |
| 2 | Canonical | The homepage emits no `<link rel="canonical">` at all | **P0** |
| 3 | Structured data | No `Organization` / `WebSite` on the site; only `BlogPosting` exists | **P0** |
| 4 | Titles | Site title 77 chars, meta description 227 chars — both truncate in SERPs | **P1** |
| 5 | Titles | Sub-page titles are bare nouns ("Products", "Insights") with no qualifier | **P1** |
| 6 | Structured data | Breadcrumbs are rendered but not marked up as `BreadcrumbList` | **P1** |
| 7 | Structured data | Client stories carry no `Article`; products carry no `Product`/`SoftwareApplication` | **P1** |
| 8 | Careers | Roles have full `JobPosting` data and no schema, and no URL of their own | **P1** |
| 9 | Titles | SEO-plugin `generateTitle` double-suffixes "\| Cogniviti Labs" | **P1** |
| 10 | OG image | Products, services and the listings all share one generic card | **P2** |
| 11 | Locale | `html lang="en"` vs `og:locale en_GB` | **P2** |
| 12 | Icons | Only `favicon.ico` — no PNG/SVG icon set, no web manifest | **P2** |
| 13 | 404 | No `not-found.tsx`, so the site's 404 is Next's bare default | **P2** |
| 14 | Sitemap | Hand-maintained route list; `changeFrequency`/`priority` are noise | **P3** |
